...
fn initConfigsFromArgs() void {
    ...
    if (temp != null) {
        ...
        if (convert_mode == 0) {
            std.debug.print("expected convert_mode is dense|spare|parts|utf8\n", .{});
            std.os.exit(1);
        }
    }
...
pub fn countNGram(step2_time: i64) !void {
    ...
    // n_gram0: chạy hết trong 1 lượt
    const thread1 = try std.Thread.spawn(.{}, NGram.countAndWrite48, .{ &gram, text, "data/24-fourth_grams.txt", "data/28-eighth_grams.txt" });
    const thread2 = try std.Thread.spawn(.{}, NGram.countAndWrite157, .{ &gram, text, "data/21-uni_grams.txt", "data/25-fifth_grams.txt", "data/27-seventh_grams.txt" });
