...
pub fn write_transforms_to_file(
    ...
    var sho_file = try std.fs.cwd().createFile(short_vi_filename, .{});
    ...
    var sho_wrt = Text.BufferedWriter{ .unbuffered_writer = sho_file.writer() };
    const sho_writer = sho_wrt.writer();
    ...
    while (i < text.tokens_num) : (i += 1) {
        ...
        if (text_utils.writeTokenInfo(tk_info, text, prev_is_syllable)) {
            ...
            const too_short = false;//text.line_tokens_count < 1024;
            // const no_vietnamese = text.line_vi_tokens_len == 0;
            const no_vietnamese = false;//text.line_bytes_len * 20 > text.line_vi_tokens_len * 100;
            const low_vietnamese = false;//text.line_bytes_len * 60 > text.line_vi_tokens_len * 100;
            var almost_viet: bool = true;//text.line_bytes_len * 70 > text.line_vi_tokens_len * 100;
